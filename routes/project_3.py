...
from models.project_3 import Project3User
from schemas.project_3 import User3Create, User3Response
...
def create_user(user:User3Create, db: Session = Depends(get_db)):
    db_user = Project3User(**user.model_dump())
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def get_user(user_id:int, db: Session = Depends(get_db)):
    user = db.query(Project3User).filter(Project3User.id == user_id).first()
    if not user:
