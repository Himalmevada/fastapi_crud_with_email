...
from models.project_2 import Project2User
from schemas.project_2 import User2Create, User2Response
...
def create_user(user:User2Create, db: Session = Depends(get_db)):
    db_user = Project2User(**user.model_dump())
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def get_user(user_id:int, db: Session = Depends(get_db)):
    user = db.query(Project2User).filter(Project2User.id == user_id).first()
    if not user:
